...
public:
	Solver_B_linear() {};
	~Solver_B_linear() {};
	void Solve(int l, svm_node * const * x_, double *b_, schar *y_,
...
void Solver_B_linear::Solve(int l, svm_node * const * x_, double *b_, schar *y_,
	double *alpha_, double *w, double Cp, double Cn, double eps, SolutionInfo* si,
	int shrinking, int qpsize)
{
	this->l = l;
	...
	double *alpha, SolutionInfo* si, double Cp, double Cn)
{
	...
	svm_node **u = new svm_node*[l];
	for(i=0;i<l;i++)
	  {
	    u[i] = new svm_node[lbp1];
	    for(j=0;j<lb;j++)
	      {
		u[i][j].index = j+1;
		u[i][j].value = Kernel::k_function(prob->x[i],prob->x[workpt[j]], *param)*y[workpt[j]];
	      }
	    u[i][lb].index = -1;
	  }
	...
	double *w = new double[lbp1];
	for (i=0;i<=lb;i++)
	  w[i] = 0;
	Solver_B_linear s;
	s.Solve(l, u, minus_ones, y, alpha, w, Cp, Cn,
	...
	for (i=0;i<lb;i++)
	  alpha[workpt[i]] = y[workpt[i]]*w[i+1];
	...
	if (param->kernel_type == LINEAR)
	{
		...
		for (i=0;i<=prob->n;i++)
			w[i] = 0;
		Solver_B_linear s;
		s.Solve(l, prob->x, minus_ones, y, alpha, w, Cp, Cn,
		param->eps, si, param->shrinking, param->qpsize);
	}
	else
	{
	...
	double *alpha, SolutionInfo* si)
{
	...
	if (param->kernel_type == LINEAR)
	{
		...
		for (i=0;i<l;i++)
			x[i] = x[i+l] = prob->x[i];
		Solver_B_linear s;
		s.Solve(2*l, x, linear_term, y, alpha2, w, param->C,
...
static decision_function solve_spoc(const svm_problem *prob, 
...
static decision_function solve_msvm(const svm_problem *prob, 
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		if(strcmp(cmd,"rho")==0)
		  {
		    ...
		    for(int p=0;p<model->nr_class*(model->nr_class-1)/2;p++)
		      fscanf(fp," %lf", &model->rho[p]);
		  } 
		...
		{
			fprintf(stderr,"unknown text %s in model file\n", cmd);
			exit(1);
		}
	}
