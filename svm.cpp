...
static void solve_rsvmssvm(
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn, int modified)
{
	int l = prob->l;
	schar *y = new schar[l];

	int lb = (int) (param->nu*l);
 	if(l >= BIG_PROB_THRESHOLD && lb >= REDUCE_THRESHOLD){//20000,1000
		lb = param->shrinking;
...
//	double *alphab = new double[lbp1];
	...
	for(j=0;j<lb;j++)
	  ...
	  for(i=0;i<l;i++)
	    {
		  Qb[j*l+i] = Kernel::k_function(prob->x[i],prob->x[j],*param)*y[i]*y[j];
		  //		  if(y[j]!=y[i])
		{
	...
	for(j=lb;j<l;j++)
	  {
	    ...
	    for(i=0;i<l;i++)
	      {
		    qtemp[i] = Kernel::k_function(prob->x[i],prob->x[j],*param)*y[i]*y[j];
		    //		    if(y[j] != y[i])
		  {
		    temp+=fabs(qtemp[i]);
		    minindex++;
		...
		workpt[minindex] = j;
		heap[minindex] = temp;
		memcpy(&(Qb[minindex*l]), qtemp,sizeof(double)*l);
	      }
	  }
	...
	{
	   for (j=0;j<lb;j++)
	      Qb[i*lbp1+j] = Kernel::k_function(prob->x[i],prob->x[workpt[j]],*param)*y[i]*y[workpt[j]];
	   Qb[i*lbp1+lb] = (double) y[i];
        ...
        {
            for(i=0;i<l;i++)
              Qb[j*l+i] = Kernel::k_function(prob->x[i],prob->x[workpt[j]],*param)*y[i]*y[workpt[j]];
        }
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn, int modified)
{
	...
	double *work = new double[lbp1*lbp1];
	int *ipiv = new int[lbp1];
	...
	for(j=0;j<lb;j++)
	  ...
	  for(i=0;i<l;i++)
	    {
		  Qbt[j*l+i] = Kernel::k_function(prob->x[i],prob->x[j],*param)*y[i]*y[j];
		  //		  if(y[j]!=y[i])
		{
	...
	for(j=lb;j<l;j++)
	  {
		...
		workpt[minindex] = j;
		heap[minindex] = temp;
		memcpy(&(Qbt[minindex*l]), qtemp,sizeof(double)*l);
	      }
	  }
#endif
	{
	...
	for (j=0;j<lb;j++)
	{
	    for(i=0;i<l;i++) 
	      {
	      	      Qbt[j*l+i] = Kernel::k_function(prob->x[i],prob->x[workpt[j]],*param)*y[i]*y[workpt[j]];
	      }
	}
	for(i=0;i<l;i++)
	...
	if (modified==1) {
		for (i=0;i<lb;i++)
			for (j=0;j<lb;j++)
				H[i*lbp1+j]+=Qbt[i*l+workpt[j]]/2/Cp;
		H[lb*lbp1+lb]+=1;
		i = lbp1*lbp1;
		j = 1;

		dsysv_("L", &lbp1, &j, H, &lbp1,ipiv,alphab,&lbp1,work,&i,&indef,1); 
		if(indef != 0)
		   printf("error! %d\n", indef); 
	}
	else
...
//		}
		...
		dtrsv_("L", "N", "N", &lbp1, H, &lbp1, alphab, &inc);
		dtrsv_("L", "T", "N", &lbp1, H, &lbp1, alphab, &inc); 
	}
	for(i=0;i<lb;i++) 
	...
	double *alpha, Solver::SolutionInfo* si, double Cp, double Cn)
{
	...
	while(1)
		...
		/*  \alphatmp = \bQ' \balpha */
		dgemv_("T", &l, &lbp1, &one, Qbt, &l, alphab, &inc, &zero, alphatmp, &inc);


		/* \alphatmp = \bQ \alphatmp + (1/2C - \beta)\alphab  -e */
		/* assume \beta = 0.95/C */
		double beta = 0.95;
		for (i=0;i<l;i++) 
			alphatmp1[i] = (0.5-beta)/Cp*alphab[i]-1;
		dgemv_("N", &l, &lbp1, &one, Qbt, &l, alphatmp, &inc, &one, alphatmp1, &inc);
		...
		for (i=0;i<l;i++) {
			alphatmp[i] = alphatmp1[i] + beta*alphab[i]/Cp;
			if (dmin > alphatmp[i]) 
				dmin = alphatmp[i];
			//			if (alphab[i] > param->eps/100 && dmax < alphatmp[i]) 
			if (alphab[i] > param->eps && dmax < alphatmp[i]) 
				dmax = alphatmp[i];
			alphamin=min(alphamin, alphab[i]);
		}
		//if (dmin-dmax >= -param->eps && alphamin >= -param->eps/100){
		if (dmin-dmax >= -param->eps && alphamin >= -param->eps){
//		if (dmin-dmax >= -param->eps*lbp1 && alphamin >= -param->eps/100*lbp1){
			printf("%g %g %g\n", dmin, dmax, alphamin);
			printf("iter = %d\n ", iter);
			break;
		}
		...
		dgemv_("T", &l, &lbp1, &one, Qbt, &l, alphatmp, &inc, &zero, alphatmp1, &inc);
		dtrsv_("L", "N", "N", &lbp1, H, &lbp1, alphatmp1, &inc);
		dtrsv_("L", "T", "N", &lbp1, H, &lbp1, alphatmp1, &inc); 
		dgemv_("N", &l, &lbp1, &one, Qbt, &l, alphatmp1, &inc, &zero, alphatmp, &inc);	
		
		/* alphab <- (I - ...)*alphab = alphab - alphatmp */
		daxpy_(&l, &mone, alphatmp, &inc, alphab, &inc);
	...
	double Cp, double Cn)
{
	...
	switch(param->svm_type)
	{
		case C_SVC:
		      ...
		      break;
		case RSVMEQ:
		      solve_rsvmeq(prob,param,alpha,&si,Cp,Cn,0);
		      break;
		case RSVMLSVM:
		      solve_rsvmlsvm(prob,param,alpha,&si,Cp,Cn);
		      break;
		case RSVMEQ2:
		      solve_rsvmeq(prob,param,alpha,&si,Cp,Cn,1);
		      break;
		case RSVMSSVM:
		      solve_rsvmssvm(prob,param,alpha,&si,Cp,Cn,0);
		      break;
		case RSVMSSVM1:
		      solve_rsvmeq(prob,param,alpha,&si,Cp,Cn,2);
		      break;			
		case NU_SVC:
...
"c_svc","rsvm_lssvm","rsvm_lsvm","rsvm_lssvm2", "rsvm_ssvm", "nu_svc","one_class","epsilon_svr","nu_svr",NULL };
