...
void exit_with_help()
{
	...
	"options:\n"
	"-h number: #sv for big rsvm problem(w/ -v: for big subproblems)\n"
	"-s svm_type : set type of SVM (default 0)\n"
	"	0 -- C-SVC\n"
	"	1: rsvm with least square svm method\n"
	"	2: rsvm with lagruangian svm method\n"
	"	3: rsvm with modified least square svm method \n"
...
//	"	4 -- nu-SVR\n"
	...
	"-n nu : set the parameter nu of nu-SVC, one-class SVM, and nu-SVR(default 0.1)\n"
...

{
	...
	double CLK_TCK = 1000.0;
        printf("user time = %f sec\n",(double) ((int) buffer.tms_utime)/CLK_TCK);
}
...
void parse_command_line(int argc, char **argv, char *input_file_name, char *model_file_name)
{
	...
	param.eps = 1e-3;
	//	param.p = 0.1;
	param.p = 5; //for SSVM beta
	param.shrinking = 200; //for big rsvm subproblem size
	param.nr_weight = 0;
